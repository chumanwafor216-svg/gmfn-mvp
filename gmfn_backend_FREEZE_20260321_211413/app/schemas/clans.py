...
# Compatibility schema for old PATCH endpoint
...
def patch_member_pool_balance_compat(
    ...
    """
    Compatibility endpoint (Stable Public API):
    Old clients/tests call:
      PATCH /clans/{clan_id}/members/{user_id}/pool  {"pool_balance": N}

    Canonical endpoint is now:
      POST /clans/{clan_id}/members/pool/set  {"user_id": X, "balance": N}
    """
    clan, membership, current_user = _require_clan_admin(clan_ctx)
    if clan.id != clan_id:
        raise HTTPException(status_code=403, detail="Not allowed")
    ...
    # tests expect numeric; keep compatibility
