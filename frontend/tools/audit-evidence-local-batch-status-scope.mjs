...
const inScopeFiles = new Set(inScopeEvidenceBatchFiles);
...
if (statusResult.error) {
  console.error(
    `Evidence local batch status-scope audit could not run git status: ${statusResult.error.message}`
  );
  process.exit(1);
}

if (statusResult.status !== 0) {
  console.error("Evidence local batch status-scope audit failed to read git status:");
...
const unexpected = changedEntries.filter(({ path }) => {
  ...
  return !outOfScopeEvidenceBatchPrefixes.some((prefix) => path.startsWith(prefix));
});

if (unexpected.length > 0) {
  console.error("Evidence local batch status-scope audit failed:");
  console.error("Unexpected changed paths outside the evidence batch manifest:");
...
console.log(
  [
    "Evidence local batch status-scope audit passed:",
    `${inScopeCount} changed path(s) are in the evidence batch manifest;`,
