...
assertContains(
  ...
  /"name": "GSN"[\s\S]*?"short_name": "GSN"[\s\S]*?"description": "Open Global Support Network from this phone\."[\s\S]*?"start_url": "\/cover\?source=pwa"[\s\S]*?"display": "browser"[\s\S]*?"display_override": \["browser"\][\s\S]*?"src": "\/gsn-app-icon-ios-180-v14\.png"[\s\S]*?"purpose": "any"[\s\S]*?"src": "\/gsn-app-icon-192-v14\.png"[\s\S]*?"purpose": "any"[\s\S]*?"src": "\/gsn-app-icon-512-v14\.png"[\s\S]*?"purpose": "any"/,
...
assertContains(
  ...
  /return \[\s*"If you are inside WhatsApp, open this page in Safari first\.",\s*"Tap Share\.",\s*"Tap Add to Home Screen\. Old icon still cropped or blank\? Remove that shortcut and add GSN again\.",\s*\];[\s\S]*?Add page to home screen or Bookmark[\s\S]*?Play Protect blocks it[\s\S]*?Show iPhone screen steps[\s\S]*?Show 3 phone steps[\s\S]*?\/gsn-app-icon-ios-180-v14\.png[\s\S]*?objectFit: "contain"/,
...
assertContains(
  "src/pages/ShopGalleryPage.tsx",
  /import OwnerOnlySurfaceNav from "\.\.\/components\/OwnerOnlySurfaceNav";[\s\S]*?import \{ APP_ROUTES, routeWithCommunity \} from "\.\.\/lib\/appRoutes";[\s\S]*?const memberSurfaceLinks = useMemo\([\s\S]*?label: "Dashboard"[\s\S]*?APP_ROUTES\.DASHBOARD[\s\S]*?label: "Community Home"[\s\S]*?routeWithCommunity\(APP_ROUTES\.COMMUNITY, ownerSurfaceCommunityId\)[\s\S]*?label: "Marketplace"[\s\S]*?routeWithCommunity\(APP_ROUTES\.MARKETPLACE, ownerSurfaceCommunityId\)[\s\S]*?label: "Public Shop"[\s\S]*?shopRootPath \|\| publicShopReturnPath[\s\S]*?label: "Finance"[\s\S]*?routeWithCommunity\(APP_ROUTES\.FINANCE, ownerSurfaceCommunityId\)[\s\S]*?label: "Loan Support"[\s\S]*?routeWithCommunity\(APP_ROUTES\.LOANS, ownerSurfaceCommunityId\)[\s\S]*?label: "Trust"[\s\S]*?routeWithCommunity\(APP_ROUTES\.TRUST, ownerSurfaceCommunityId\)[\s\S]*?<OwnerOnlySurfaceNav[\s\S]*?label="Your shop shortcuts"[\s\S]*?ariaLabel="Your Public Shop shortcuts"[\s\S]*?links=\{memberSurfaceLinks\}[\s\S]*?requireOwnerMatch=\{true\}/,
  "Public Shop owner shortcuts must route the shop owner to Dashboard, Community Home, Marketplace, Public Shop, Finance, Loan Support, and Trust while staying hidden from public visitors and non-owner signed-in members."
);
...
assertContains(
  "src/pages/ShopGalleryPage.tsx",
  /sourceProductId\?: number;[\s\S]*?sourceProductBlock\?: number;[\s\S]*?sourceProductSlotNumber\?: number;[\s\S]*?sourceProductId:[\s\S]*?src\?\.source_product_id \|\| src\?\.sourceProductId[\s\S]*?sourceProductBlock:[\s\S]*?src\?\.source_product_block \|\| src\?\.sourceProductBlock[\s\S]*?sourceProductSlotNumber:[\s\S]*?src\?\.source_product_slot_number \|\| src\?\.sourceProductSlotNumber[\s\S]*?sourceShopWhatsApp: firstMeaningful\(miniSpotlight\?\.sourceShopWhatsApp\)[\s\S]*?function contactSpotlightOwnerByWhatsApp\(\)[\s\S]*?miniSpotlightView\.sourceShopWhatsApp[\s\S]*?debugId="shop-gallery\.spotlight\.whatsapp-chat"/,
  "Public Shop Spotlight must preserve broadcast source community/product/block fields and attach the single WhatsApp action to the rotating source shop instead of the retired Open/Explore redirect."
);
