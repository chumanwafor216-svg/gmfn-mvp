...
const MarketplaceMembersSection = lazy(
  () => import("./marketplace/MarketplaceMembersSection")
);

const MarketplaceSupportSection = lazy(
  () => import("./marketplace/MarketplaceSupportSection")
);
...
const MarketplaceToolsSection = lazy(
  () => import("./marketplace/MarketplaceToolsSection")
);

const MarketplaceTradeEvidenceSection = lazy(
  () => import("./marketplace/MarketplaceTradeEvidenceSection")
);
...
function marketplaceOsSectionStyle(isCompact: boolean): React.CSSProperties {
  return {
    ...pageCard("#FFFFFF"),
    order: 2,
    padding: isCompact ? "20px 14px 14px" : 18,
  };
}
...
function marketplaceHeroShellStyle(isCompact: boolean): React.CSSProperties {
  return {
    ...
    padding: isCompact ? "18px 12px 14px" : 20,
...
function marketplaceHeroTitleStyle(isCompact: boolean): React.CSSProperties {
  return {
    color: "#FFFFFF",
    fontSize: isCompact ? 21 : 32,
    fontWeight: 950,
    lineHeight: isCompact ? 1.06 : 1.02,
    letterSpacing: 0,
    overflowWrap: "break-word",
  };
}
...
function marketplaceHeroStatIconStyle(
  bg: string,
  isCompact: boolean
): React.CSSProperties {
  return {
...
function marketplaceHeroStatValueStyle(
  isCompact: boolean,
  value: string
): React.CSSProperties {
    ...
    try {
      await addProtectedTradeEvent(tradeId, {
        ...
        meta: {
          ...
          boundary:
            "Outcome evidence only. Not a public review, not a personal label, not escrow, not automatic payout, not a bank guarantee, not a delivery guarantee.",
        },
      });
  ...
  return (
    <MarketplaceShell isCompact={isCompact}>
          ...
          <div style={marketplaceHeroStatsStyle(isCompact)}>
            {marketplaceStats.map((item) => (
              <div key={item.label} style={marketplaceHeroStatCellStyle(isCompact)}>
                <span
                  aria-hidden="true"
                  style={marketplaceHeroStatIconStyle(item.tone, isCompact)}
                >
      ...
      {sectionsOpen.board || sectionsTouched.board ? (
        <Suspense fallback={null}>
          <MarketplaceBoardSection
            isCompact={isCompact}
            isOpen={sectionsOpen.board}
            activeCommunityId={activeCommunityId}
            activeCommunityName={activeCommunityName}
            canPostMarketplaceNotice={canPostMarketplaceNotice}
            canManageMarketplaceNoticeSettings={canManageMarketplaceNoticeSettings}
            activeNoticePostingPolicy={activeNoticePostingPolicy}
            marketplaceNoticeSettingsSaving={marketplaceNoticeSettingsSaving}
            marketplaceNoticesLoading={marketplaceNoticesLoading}
      ...
      {sectionsOpen.money || sectionsTouched.money ? (
        <Suspense fallback={null}>
          <MarketplaceMoneySection
            isCompact={isCompact}
            isOpen={sectionsOpen.money}
            visiblePoolAmount={visiblePoolAmount}
            visiblePoolCurrency={visiblePoolCurrency}
            communitySettlementReady={communitySettlementReady}
            payoutReady={payoutReady}
            moneySurface={moneySurface}
            marketplaceMoneyOutTo={marketplaceMoneyOutTo}
            payInEditorOpen={payInEditorOpen}
            payInAccountDraft={payInAccountDraft}
            savingPayInAccount={savingPayInAccount}
            onToggleMoney={(event) => toggleSectionFromButton(event, "money")}
            onTogglePayInEditor={() => {
              pendingMarketplaceSectionRef.current = "";
              cancelMarketplaceSectionScroll();
              setPayInEditorOpen((value) => !value);
            }}
            onUpdatePayInAccountDraft={updatePayInAccountDraft}
            onSavePayInAccount={() => {
              void savePayInAccount();
            }}
            onClosePayInEditor={() => setPayInEditorOpen(false)}
      ...
      {sectionsOpen.rosca ? (
        <Suspense fallback={null}>
          <MarketplaceRoscaSection
            isCompact={isCompact}
            isOpen={sectionsOpen.rosca}
            sectionRef={roscaSectionRef}
            roscaCyclesFeatureOff={roscaCyclesFeatureOff}
            roscaCyclesFeatureOffText={roscaCyclesFeatureOffText}
            roscaYearlyActive={roscaYearlyActive}
            roscaTitle={roscaTitle}
            roscaContributionAmount={roscaContributionAmount}
            roscaCurrency={roscaCurrency}
            roscaIntervalDays={roscaIntervalDays}
            selectedRoscaMemberIds={selectedRoscaMemberIds}
            selectedRoscaMemberSet={selectedRoscaMemberSet}
            selectedRoscaMembers={selectedRoscaMembers}
            roscaSelectableMembers={roscaSelectableMembers}
            creatingRoscaPackage={creatingRoscaPackage}
            startingRoscaCycle={startingRoscaCycle}
            recordingRoscaPayoutKey={recordingRoscaPayoutKey}
            latestRoscaCycle={latestRoscaCycle}
            nextRoscaPayoutRound={nextRoscaPayoutRound}
            roscaPackage={roscaPackage}
            latestRoscaPackagePayment={latestRoscaPackagePayment}
            memberCount={members.length}
            pageCard={pageCard}
            marketplaceOsIconStyle={marketplaceOsIconStyle}
            badge={badge}
            marketplaceActionStyle={marketplaceActionStyle}
            sectionLabel={sectionLabel}
            helperText={helperText}
            innerCard={innerCard}
            inputStyle={inputStyle}
            marketplaceFieldTouchProps={marketplaceFieldTouchProps}
            stableStatusPillStyle={stableStatusPillStyle}
            marketplaceSurfaceTouchProps={marketplaceSurfaceTouchProps}
            marketplaceInlineActionsStyle={marketplaceInlineActionsStyle}
            marketplaceInlineActionStyle={marketplaceInlineActionStyle}
            marketplaceProfileStatStyle={marketplaceProfileStatStyle}
            runMarketplaceAction={runMarketplaceAction}
            showNotice={showNotice}
            setRoscaTitle={setRoscaTitle}
            setRoscaContributionAmount={setRoscaContributionAmount}
            setRoscaCurrency={setRoscaCurrency}
            setRoscaIntervalDays={setRoscaIntervalDays}
            setSelectedRoscaMemberIds={setSelectedRoscaMemberIds}
            onToggleRosca={(event) => toggleSectionFromButton(event, "rosca")}
            createRoscaYearlyInstruction={createRoscaYearlyInstruction}
            startMarketplaceRoscaCycle={startMarketplaceRoscaCycle}
            recordMarketplaceRoscaPayout={recordMarketplaceRoscaPayout}
            onUploadedRoscaPackagePayment={(payment) => {
              setCreatedRoscaPackageInstruction(payment as ExpectedPaymentRecord);
      ...
      {sectionsOpen.members ? (
        <Suspense fallback={null}>
          <MarketplaceMembersSection
            isCompact={isCompact}
            memberRows={memberRows}
            visibleTradeMemberRows={visibleTradeMemberRows}
            hiddenTradeMemberRows={hiddenTradeMemberRows}
