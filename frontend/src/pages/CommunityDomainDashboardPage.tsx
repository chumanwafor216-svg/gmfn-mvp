...
export default function CommunityDomainDashboardPage() {
  ...
  const [beneficiaryDeliveryPackByOutcomeId, setBeneficiaryDeliveryPackByOutcomeId] =
    useState<Record<string, RealLifeBeneficiaryOutcomeRelatedRecord>>({});
  ...
  async function createBeneficiaryOutcomeConfirmationLink(outcomeEventId: string) {
    ...
    try {
      ...
      const payloadRecord = isUnknownRecord(payload) ? payload : {};
      const path = cleanText(payloadRecord.public_path);
      const shareText = path ? `${window.location.origin}${path}` : "";
      const deliveryPack = isUnknownRecord(payloadRecord.delivery_pack)
        ? payloadRecord.delivery_pack
        : {};
  ...
  async function recordBeneficiaryOutcomeContactConsent(item: RealLifeBeneficiaryOutcomeRow) {
    const requestDomainId = cleanText(domain?.id || communityDomainId);
  ...
  async function withdrawBeneficiaryOutcomeContactConsent(item: RealLifeBeneficiaryOutcomeRow) {
    const requestDomainId = cleanText(domain?.id || communityDomainId);
    const outcomeEventId = cleanText(item?.event_id);
  ...
  async function recordBeneficiaryOutcomeDeliveryReceipt(item: RealLifeBeneficiaryOutcomeRow) {
    const requestDomainId = cleanText(domain?.id || communityDomainId);
    const outcomeEventId = cleanText(item?.event_id);
    const receiptDraft =
  ...
  async function correctBeneficiaryOutcomeDeliveryReceipt(item: RealLifeBeneficiaryOutcomeRow) {
    const requestDomainId = cleanText(domain?.id || communityDomainId);
    const outcomeEventId = cleanText(item?.event_id);
  ...
  async function checkBeneficiaryOutcomeProviderSend(item: RealLifeBeneficiaryOutcomeRow) {
    const requestDomainId = cleanText(domain?.id || communityDomainId);
    const outcomeEventId = cleanText(item?.event_id);
    const localDeliveryPack = beneficiaryDeliveryPackByOutcomeId[outcomeEventId] || {};
    const deliveryEventId = cleanText(
    ...
    try {
      ...
      const sent = isUnknownRecord(payload) && Boolean(payload.external_channels_sent_by_gsn);
      setMessage(
        sent
  ...
  async function submitBeneficiaryOutcomeCorrectionReview(item: RealLifeBeneficiaryOutcomeRow) {
    const requestDomainId = cleanText(domain?.id || communityDomainId);
    const outcomeEventId = cleanText(item?.event_id);
    if (!requestDomainId || !outcomeEventId) {
            ...
            >
              <CommunityDomainSetupOverviewPanel
                data={{
                  ...
                  onOpenNoticeModal: () => setDomainNoticeModalOpen(true),
            ...
            <div style={whiteCard()}>
              <div style={{ display: "grid", gap: 12 }}>
                ...
                {!isActiveLaneReadinessLoading && showActiveDomainSettingsSummary ? (
                  ...
                  >
                    <CommunityDomainOperatingSummaryPanel
                      data={{
                        ...
                        helperText,
                        onEditSetupDetails: () => openSetupJourney("edit"),
                        onOpenLiveLane: () => {
                          focusWorkSurfaceAfterOpenRef.current = true;
                          setShowAdvancedTools(true);
                          setOperatingAreaPickerOpen(false);
                ...
                {!isActiveLaneReadinessLoading && activeLane === "modules" ? (
                  <Suspense
                    fallback={
                      <div style={{ ...helperText(), marginTop: 4 }}>
                        Loading service focus...
                      </div>
                    }
                  >
                    <CommunityDomainServiceFocusPanel
                      data={{
                        activeServiceDetail,
                        appealReadiness,
