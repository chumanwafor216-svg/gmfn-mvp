...
type TrustSlipDecisionPackHousingReferencePointer = {
  key: string;
  label: string;
  status: string;
  value: string;
...
type TrustSlipDecisionPackEvidenceExtract = {
  ...
  housingReferencePointers: TrustSlipDecisionPackHousingReferencePointer[];
...
function normalizeTrustSlipDecisionPackEvidence(raw: any): TrustSlipDecisionPackEvidenceExtract | null {
  ...
  const recordPointers = Array.isArray(extract?.record_pointers)
    ? extract.record_pointers
        ...
        .slice(0, 4)
    : [];
  const housingReferencePointers = Array.isArray(extract?.housing_reference_pointers)
    ? extract.housing_reference_pointers
        ...
        .filter((row: TrustSlipDecisionPackHousingReferencePointer) => row.key || row.label || row.value)
        .slice(0, 4)
    : [];
  ...
  const confirmationPointers = Array.isArray(extract?.confirmation_pointers)
    ? extract.confirmation_pointers
        .map((row: any) => ({
          key: firstTruthy(row?.key),
          label: firstTruthy(row?.label, "Community witness outcome"),
          status: firstTruthy(row?.status),
          value: firstTruthy(row?.value),
          source: firstTruthy(row?.source),
          evidenceCount: Number(row?.evidence_count ?? row?.evidenceCount ?? 0) || 0,
  ...
  return {
    ...
    housingReferencePointers,
    housingReferenceBoundaryNote: firstTruthy(extract?.housing_reference_boundary_note, raw?.housing_reference_boundary_note),
...
export default function TrustSlipPage() {
  ...
  const privateDecisionPackHousingReferencePointers = (decisionPackEvidenceExtract?.housingReferencePointers || []).slice(0, 3);
            ...
            {!collapsed.decisionPackPrivatePreview ? (
              <React.Suspense
                fallback={
                  ...
                  >
                    Loading private Decision Pack preview...
                  </div>
                }
              >
                <TrustSlipDecisionPackPrivatePreview
                  isCompact={isCompact}
                  isCollapsed={collapsed.decisionPackPrivatePreview}
                  decisionPackEvidenceLoading={decisionPackEvidenceLoading}
                  privateDecisionPackEvidenceAvailable={privateDecisionPackEvidenceAvailable}
                  privateDecisionPackEvidenceScopeSummary={privateDecisionPackEvidenceScopeSummary}
                  privateDecisionPackEvidenceScopeBoundary={privateDecisionPackEvidenceScopeBoundary}
                  decisionPackEvidenceExtract={decisionPackEvidenceExtract}
                  privateDecisionPackDeclaredClaims={privateDecisionPackDeclaredClaims}
                  privateDecisionPackRecordPointers={privateDecisionPackRecordPointers}
                  privateDecisionPackHousingReferencePointers={privateDecisionPackHousingReferencePointers}
                  privateDecisionPackGuaranteeOutcomePointers={privateDecisionPackGuaranteeOutcomePointers}
                  privateDecisionPackFulfillmentOutcomePointers={privateDecisionPackFulfillmentOutcomePointers}
                  privateDecisionPackCompletedWorkPointers={privateDecisionPackCompletedWorkPointers}
                  privateDecisionPackDemandRequestOutcomePointers={privateDecisionPackDemandRequestOutcomePointers}
                  privateDecisionPackConfirmationPointers={privateDecisionPackConfirmationPointers}
                  privateDecisionPackIssueResolutionPointers={privateDecisionPackIssueResolutionPointers}
                  privateDecisionPackEvidenceCategories={privateDecisionPackEvidenceCategories}
                  selectedPurposeKey={selectedPurposeOption.key}
                  housingExternalContactLabel={housingExternalContactLabel}
                  setHousingExternalContactLabel={setHousingExternalContactLabel}
                  housingExternalContactChannel={housingExternalContactChannel}
                  setHousingExternalContactChannel={setHousingExternalContactChannel}
                  housingExternalContactValue={housingExternalContactValue}
                  setHousingExternalContactValue={setHousingExternalContactValue}
                  copyDecisionPackConsentSummary={copyDecisionPackConsentSummary}
                  copyDecisionPackConsentJson={copyDecisionPackConsentJson}
                  decisionPackConsentShares={decisionPackConsentShares}
                  decisionPackAccesses={decisionPackAccesses}
                />
              </React.Suspense>
            ) : null}
          </section>
