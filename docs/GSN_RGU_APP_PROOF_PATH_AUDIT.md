...
The app has the right route structure for the workshop story. The original local admin account did not prove the strongest claim, so a dedicated workshop demo account has now been prepared.
...
That account should not be used as the main workshop proof account.

The workshop proof account is:
...
Current verified proof state for that account:
...
- TrustSlip: shorter portable proof layer.
- TrustSlip Verify: public validity check.

This is good. It means we do not need to invent a new demo structure. We need to clean and strengthen the actual proof.
...
- TrustSlip proves the shorter portable state.
...
This is useful for testing warnings, but not useful for proving that a person can carry trust from Lagos to Aberdeen. The workshop needs a prepared demo member with credible positive evidence.
...
**Does this prove portable trust, or is it noise?**
...
8. Tested the proof APIs on laptop.
...
1. Prepare backup screenshots for each proof step.
...
The live app should not yet be treated as fully workshop-ready until the trust proof path is corrected with an active TrustSlip and working merchant verification.
...
Do not demo the old local admin account as the main proof of GSN. It demonstrates caution, expiry, and inactive merchant verification more than it demonstrates portable trust. Use the Amara workshop demo account.

---

## 8. Immediate Next Work Order

1. Finish phone audit using the same proof path.
