...
- Backend parity: backend Decision Pack definitions now carry the same evidence/source/gap/boundary matrix, controlled purpose-specific community confirmation prompts, public-safe declared work/service claim pointers for relevant packs, public-safe financial/support record pointers for relevant packs, aggregate guarantee/support outcome pointers, aggregate community witness outcome pointers, and aggregate issue-resolution pointers. The remaining work is deeper data capture: dedicated structured claims, customer-confirmed completed jobs, landlord references, mature issue-resolution summaries, mature guarantee outcome summaries, supplier fulfilment outcomes, and richer witness-answer workflows.
