...
- Community Bulletin:
  ...
  - ordinary Community notices keep the main board short, but officers may attach longer public-safe full notice details and request a public QR link; QR-enabled notices expose `Open QR link` inside Reactions, public URL attachments expose one `Open attachment` action, and the public route must hide private sender/contact/roll-call data
  - Bulletin attachments in the current notice engine are public `http/https` links only; direct gallery/file upload requires governed media storage before it can be offered as a real upload feature
